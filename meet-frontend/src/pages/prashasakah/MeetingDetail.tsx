  ...
  if (!meeting) {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-semibold text-surface-800 mb-2">Meeting not found</h2>
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-surface-500">
        ...
        <Link to="/prashasakah/meetings" className="hover:text-surface-600">Meetings</Link>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
        <span className="text-surface-800 font-medium">{meeting.roomTitle || meeting.roomName}</span>
      </nav>
          ...
          <div className="bg-white rounded-xl border border-surface-200 p-5">
            ...
            <dl className="space-y-4">
              <div>
                ...
                <dd className="mt-1 text-sm font-medium text-surface-800 font-mono">
                  {meeting.roomName}
                </dd>
              </div>
              <div>
                <dt className="text-sm text-surface-500">Duration</dt>
                <dd className="mt-1 text-sm font-medium text-surface-800">
                  {formatDuration(meeting.duration)}
                </dd>
              </div>
              <div>
                <dt className="text-sm text-surface-500">Participants</dt>
                <dd className="mt-1 text-sm font-medium text-surface-800">
              ...
              <div>
                <dt className="text-sm text-surface-500">Ended</dt>
                <dd className="mt-1 text-sm font-medium text-surface-800">
          ...
          <div className="bg-white rounded-xl border border-surface-200 p-5">
            ...
            <dl className="space-y-4">
              <div className="flex justify-between">
                <dt className="text-surface-500">Total Participants</dt>
                <dd className="font-semibold text-surface-800">{participants.length}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-surface-500">Unique Users</dt>
                <dd className="font-semibold text-surface-800">
                  {new Set(participants.filter(p => p.userId).map(p => p.userId)).size}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-surface-500">Guest Users</dt>
                <dd className="font-semibold text-surface-800">
                  {participants.filter(p => !p.userId).length}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-surface-500">Chat Messages</dt>
                <dd className="font-semibold text-surface-800">{chatMessages.length}</dd>
              </div>
            </dl>
          </div>
        </div>
                ...
                <button
                  ...
                  className={`px-6 py-4 text-sm font-medium border-b-2 transition-colors ${
                    activeTab === 'participants'
                      ...
                      : 'border-transparent text-surface-500 hover:text-surface-600 hover:border-surface-300'
                  }`}
                >
                              ...
                              <tr key={index} className="hover:bg-surface-50">
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <div className="flex items-center">
                                    ...
                                    <div className="ml-3">
                                      ...
                                      {participant.email && (
                                        <div className="text-xs text-surface-500">{participant.email}</div>
                                      )}
                                    </div>
                                  </div>
                                </td>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-surface-500">
                                  {participant.leftAt 
                                    ...
                                    : <span className="text-success-600">Still in meeting</span>
                                  }
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-surface-500">
                                  {durationMins}m
      ...
      <div className="pt-4">
        <Link 
          ...
          className="inline-flex items-center text-sm text-surface-500 hover:text-surface-600"
        >
