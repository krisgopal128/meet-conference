...
function PanelLoader() {
  return (
    <div className="w-80 flex items-center justify-center bg-surface-800 border-l border-surface-700">
...
function ConferenceRoomInner(_props: ConferenceRoomProps) {
  ...
  // - Fresh processor: creates new instance each time (avoids stale state)
  useEffect(() => {
    const cameraPublication = room.localParticipant.getTrackPublication(Track.Source.Camera);
    const videoTrack = cameraPublication?.track;
    
    // Skip if no camera track
    ...
    // Track this effect instance for cleanup
    let cancelled = false;
    ...
    const applyBlurChange = async () => {
      ...
      const success = await toggleBlur(track, backgroundBlurEnabled);
      ...
      <div className="flex flex-col h-screen bg-surface-900 text-white overflow-hidden">
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 min-w-0 p-3 flex flex-col">
            {/* Lobby notification for moderators */}
            {meetingRoomConfig.features.inlineLobbyBanner && isModerator && lobbyCount > 0 && (
              <div className="mb-3 rounded-lg border border-warning-200 dark:border-warning-700 bg-warning-50 dark:bg-warning-900/20 px-4 py-3 flex items-center justify-between gap-3 animate-fade-in">
                ...
                <button
                  ...
                  className="btn-secondary btn-sm shrink-0 bg-white dark:bg-surface-700 border-warning-200 dark:border-warning-700 text-warning-700 dark:text-warning-300 hover:bg-warning-100 dark:hover:bg-surface-600"
                >
                  Review
                </button>
              </div>
